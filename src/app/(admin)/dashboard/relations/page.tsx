        ...
        <div className="space-y-6">
            ...
            <div>
                <h1 className="text-2xl font-bold text-zinc-900 dark:text-white flex items-center gap-2">
                    <svg className="w-6 h-6 text-purple-500 md:hidden" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 10h16M4 14h16M4 18h16" />
                    </svg>
                    關聯表
                </h1>
                <p className="text-zinc-500 dark:text-white/50 mt-1 hidden sm:block">查看物業、業主與租金的完整關聯樹</p>
            </div>
                    ...
                    data.map((property, index) => (
                        ...
                        >
                            ...
                            <div
                                ...
                                className="flex items-center justify-between p-3 sm:p-4 cursor-pointer hover:bg-zinc-50 dark:hover:bg-white/5 transition-colors"
                            >
                                <div className="flex items-center gap-2 sm:gap-4">
                                    <motion.div
                                        animate={{ rotate: expandedIds.has(property.id!) ? 90 : 0 }}
                                        className="text-zinc-400 dark:text-white/50 flex-shrink-0"
                                    >
                                    ...
                                    <div className="p-2 rounded-xl bg-purple-50 dark:bg-purple-500/20 flex-shrink-0 hidden xs:flex">
                                    ...
                                    <div className="min-w-0">
                                        <h3 className="text-zinc-900 dark:text-white font-semibold flex items-center gap-2 truncate">
                                            {property.name}
                                            <span className="text-zinc-400 dark:text-white/40 text-xs sm:text-sm font-normal">({property.code})</span>
                                        </h3>
                                        <p className="text-zinc-500 dark:text-white/50 text-[10px] sm:text-sm truncate">{typeLabels[property.type]}</p>
                                    </div>
                                </div>

                                <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0">
                                    <span className={`px-2 py-0.5 rounded-full text-[10px] sm:text-xs font-medium border capitalize ${statusColors[property.status]}`}>
                                        {property.status === 'holding' ? '持有' : property.status === 'renting' ? '出租' : '其他'}
                                    </span>
                                    <div className="flex items-center gap-1 text-zinc-400 dark:text-white/40 text-[10px] sm:text-sm">
                                        {property.rents.length > 0 && (
                                            <span className="flex items-center gap-1 px-1.5 py-0.5 bg-zinc-50 dark:bg-white/5 rounded">
                            ...
                            {expandedIds.has(property.id!) && (
                                ...
                                >
                                    <div className="p-3 sm:p-4 pl-8 sm:pl-16 space-y-4">
                                        {/* Property Details */}
                                        <div className="grid grid-cols-1 xs:grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                                            <div>
                                                <p className="text-zinc-400 dark:text-white/40 text-[10px] uppercase font-bold tracking-wider">{t('地址', 'Address')}</p>
                                                <p className="text-zinc-700 dark:text-white/80 line-clamp-2">{property.address || '-'}</p>
                                            </div>
                                            <div>
